...
class HubDatabase:
    def __init__(self, db_path: str, xml_dir: str):
        ...
        with _db_init_lock:
            for stmt in [s.strip() for s in DB_SCHEMA.split(';') if s.strip()]:
    ...
    def salvar_nota(self, xml_content: str, tiny_id: str = "") -> Optional[str]:
        ...
        dados = (
            chave, meta.get("numero"), meta.get("serie"), meta.get("data_emissao"),
            meta.get("tipo", "saida"), meta.get("situacao", ""), meta.get("valor_total"),
            meta.get("cnpj_emitente"), meta.get("nome_emitente"), meta.get("cnpj_dest"),
            meta.get("nome_dest"), meta.get("municipio_dest"), meta.get("uf_dest"),
            meta.get("natureza_op"), str(xml_path), xml_hash, tiny_id, agora,
        )
